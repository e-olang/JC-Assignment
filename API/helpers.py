...
def read_cases(original_df):
    ...
    return cases    # a list of dataframes, which can then be processed individually for loading into DB.
...
def create_case_combined(case_df):
    ...
    # Should only be one as a cme / drill case happens at once loc. also depends earlier delteion of similar empty columns
    ...
    # Default all non common items to Null. To be filled individual after rescaning current row (case) being processed
    blank_keys = [
    ...
    return [mapped_values]              # retunr as list for easier itteration when inserting into db

# Handle Missing Cell Per Column:
# CME Completion Date e.g. Case 3 in test file: Kilifi County
